function G = weighted_directed_graph(names,coord_x,coord_y,ind)             % Costruisco il grafo sfruttando i vettori che contengono
    ...
    %if isempty(adj{i}.search(j))
        tmp.peso = peso; tmp.nodo = j;
        adj{i}.add(tmp);        
    %else
    ...
    function peso = peso_arco(i,j) % peso_arco(i,j): restituisce il peso dell'arco dal nodo i al nodo j
